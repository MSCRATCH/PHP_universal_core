...
<div class="template_wrapper_title"><h2><i class="fa-solid fa-envelope"></i> <?php echo sanitize_1($text_templates['user_activity_log_template_notice_comment_restored']);?> <?php echo sanitize_1($article_title);?></h2>
<p class="paragraph_nm"><a class="b_link" target="_blank" href="<?= BASE_URL ?>profile/<?php echo sanitize_1($row['actor_username']);?>"><?php echo sanitize_3($row['actor_username']);?></a> <?php echo sanitize_1($text_templates['user_activity_log_template_comment_restored']);?> <?php echo sanitize_1($text_templates['user_activity_log_template_at']);?> <?php echo sanitize_1($row['user_activity_timestamp']);?></p>
<?php } else { ?>
<div class="template_wrapper_title"><h2><i class="fa-solid fa-envelope-open"></i> <?php echo sanitize_1($text_templates['user_activity_log_template_notice_comment_restored']);?> <?php echo sanitize_1($article_title);?></h2>
<p class="paragraph_nm"><a class="b_link" target="_blank" href="<?= BASE_URL ?>profile/<?php echo sanitize_1($row['actor_username']);?>"><?php echo sanitize_3($row['actor_username']);?></a> <?php echo sanitize_1($text_templates['user_activity_log_template_comment_restored']);?> <?php echo sanitize_1($text_templates['user_activity_log_template_at']);?> <?php echo sanitize_1($row['user_activity_timestamp']);?></p>
<?php } ?>
</div>
